...
export async function instantiate(imports={}, runInitializer=true) {
    ...
    const js_code = {
        ...
        'com.ghost.serialization.jsArrayToInt32ViewRaw' : (arr) => { if (arr.byteOffset % 4 !== 0) return new Int32Array(arr.slice().buffer); return new Int32Array(arr.buffer, arr.byteOffset, arr.byteLength >> 2); },
        'com.ghost.serialization.getJsInt32Raw' : (arr, index) => arr[index],
        'com.ghost.serialization.getJsArrayLengthRaw' : (arr) => arr.length,
        'com.ghost.serialization.getJsArrayByteRaw' : (arr, index) => arr[index],
    ...
    const wasmFilePath = './ghost-standalone-wasm-js.wasm';
    const importObject = {
        js_code,
        intrinsics: {
    ...
    try {
      ...
      if (isBrowser) {
        wasmInstance = (await WebAssembly.instantiateStreaming(fetch(new URL('./ghost-standalone-wasm-js.wasm',import.meta.url).href), importObject)).instance;
      }
    } catch (e) {
